...
def test_defaults_to_pane_command_selection(tmp_path) -> None:
    assert ContextMenuOrderStore(make_settings(tmp_path)).order == DEFAULT_ORDER


def test_saved_order_survives_a_restart(tmp_path) -> None:
    store = ContextMenuOrderStore(make_settings(tmp_path))
    store.save([SECTION_COMMAND, SECTION_SELECTION, SECTION_PANE])

    reopened = ContextMenuOrderStore(make_settings(tmp_path))

    assert reopened.order == [SECTION_COMMAND, SECTION_SELECTION, SECTION_PANE]
